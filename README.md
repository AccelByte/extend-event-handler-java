...
This repository contains a `sample event handler app` written in `Java`. 
...
2. Run this `extend-event-handler-java` sample app.
...
3. Install `grpcui`, please refer to the official doc on the installation, and then run this command
   ...
   with `localhost:6565` is the address for our `extend-helper-handler-java`, now go to the Web UI with 
